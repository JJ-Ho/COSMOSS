function hL = Plot1D(hAx,OneD_Data,GUI_Inputs)
%% Plot1D
%  
...
hold(hAx,'on')
    ...
    hL = plot(hAx,Res_Freq,PlotY,'-','LineWidth',2)
    if eq(PlotStick,1)
        line(hAx,[Res_Freq;Res_Freq],[zeros(1,length(Stick));Stick']);
    end
hold(hAx,'off')
...
YMax = max(abs(PlotY(:)));
hAx.YLim = [-YMax,YMax]*1.1;
hAx.YTick = linspace(-YMax,YMax,10);
YTickLabel = cell(size(hAx.YTick));
YTickLabel{1}   = sprintf('%3.2e',-YMax);
YTickLabel{end} = sprintf('%3.2e', YMax);
...
Title_String{1} = ['Coupling:',Coupling_Reg];
